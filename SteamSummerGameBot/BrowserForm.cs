        ...
        private void wbAuth_DocumentCompleted(object sender, WebBrowserDocumentCompletedEventArgs e)
        {
            ...
            if (url != "https://steamcommunity.com/login/home/?goto=my/profile" && url != "https://store.steampowered.com//login/transfer" && url.StartsWith("javascript:") == false && url.StartsWith("about:") == false)
            {
                ...
                foreach (Cookie cookie in cookies)
                {
                    ...
                    {
                       accountInfo.SessionIdCookieValue = cookie.Value;
                    }

                    // Save the "steamLogin" cookie and construct and save the user's profile link
                    else if (cookie.Name == "steamLogin")
                    {
                        accountInfo.SteamLoginCookieValue = cookie.Value;
                        accountInfo.SteamId = cookie.Value.Substring(0, 17);
                    }
                }
                try
                {
                    //Теперь нужно получить токен доступа - http://steamcommunity.com/minigame/gettoken
                    var client = new RestClient("http://steamcommunity.com/minigame/");
                    var request = new RestRequest("gettoken", Method.GET);
                    request.AddParameter("sessionid", accountInfo.SessionIdCookieValue, ParameterType.Cookie);
                    request.AddParameter("steamLogin", accountInfo.SteamLoginCookieValue, ParameterType.Cookie);
                    var response = client.Execute(request);

                    dynamic tokenInfo = JObject.Parse(response.Content);
                    if (tokenInfo.token != null)
                    {
                        var accessToken = tokenInfo.token;
                        var accountName = tokenInfo.persona_name;
                        accountInfo.AccessToken = accessToken;
                        accountInfo.Name = accountName;
                        _bot.SetAccountInfo(accountInfo);
                        Close();
                    }
                    else
                    {
